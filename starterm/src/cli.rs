...
/// CLI options for the main Starterm executable.
...
pub struct Options {
    ...
    /// X11 window ID to embed Starterm within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
    ...
    /// Specify alternative configuration file [default: %APPDATA%\starterm\starterm.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
pub struct WindowIdentity {
    /// Defines the window title [default: Starterm].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: Starterm].
...
/// Send a message to the Starterm socket.
#[cfg(unix)]
#[derive(Args, Debug)]
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "starterm.bash"),
            (Shell::Fish, "starterm.fish"),
            (Shell::Zsh, "_starterm"),
        ] {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "starterm", &mut generated);
        ...
        //
        // let mut file = File::create("../extra/completions/starterm.bash").unwrap();
        // clap_complete::generate(Shell::Bash, &mut clap, "starterm", &mut file);
        // let mut file = File::create("../extra/completions/starterm.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "starterm", &mut file);
        // let mut file = File::create("../extra/completions/_starterm").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "starterm", &mut file);
    }
}
