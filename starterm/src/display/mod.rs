...
use starterm_terminal::event::{EventListener, OnResize, WindowSize};
use starterm_terminal::grid::Dimensions as TermDimensions;
use starterm_terminal::index::{Column, Direction, Line, Point};
use starterm_terminal::selection::Selection;
use starterm_terminal::term::cell::Flags;
use starterm_terminal::term::{
    self, LineDamageBounds, Term, TermDamage, TermMode, MIN_COLUMNS, MIN_SCREEN_LINES,
};
use starterm_terminal::vte::ansi::{CursorShape, NamedColor};
...
impl Display {
    pub fn new(
        ...
        // Always focus new windows, even if no Starterm window is currently focused.
        #[cfg(target_os = "macos")]
        window.focus_window();
    ...
    pub fn draw<T: EventListener>(
        ...
        // Add damage from starterm's UI elements overlapping terminal.
        ...
        if matches!(self.raw_window_handle, RawWindowHandle::Xcb(_) | RawWindowHandle::Xlib(_)) {
            ...
            // will block to synchronize (this is `glClear` in Starterm), which causes a
