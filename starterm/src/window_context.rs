...
use starterm_terminal::event::Event as TerminalEvent;
use starterm_terminal::event_loop::{EventLoop as PtyEventLoop, Msg, Notifier};
use starterm_terminal::grid::{Dimensions, Scroll};
use starterm_terminal::index::Direction;
use starterm_terminal::sync::FairMutex;
use starterm_terminal::term::test::TermSize;
use starterm_terminal::term::{Term, TermMode};
...
/// Event context for one individual Starterm window.
...
impl WindowContext {
    ...
    fn new(
        ...
        // Create context for the Starterm window.
        Ok(WindowContext {
            preserve_title,
            terminal,
            display,
    ...
    pub fn draw(&mut self, scheduler: &mut Scheduler) {
        ...
        if !self.display.visual_bell.completed() {
            // We can get an OS redraw which bypasses starterm's frame throttling, thus
