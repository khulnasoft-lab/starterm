...
pub fn spawn_ipc_socket(
    ...
    env::set_var(STERTERM_SOCKET_ENV, socket_path.as_os_str());
    if options.daemon {
        println!("STERTERM_SOCKET={}; export STERTERM_SOCKET", socket_path.display());
    }
...
/// Send a message to the active Starterm socket.
...
fn find_socket(socket_path: Option<PathBuf>) -> IoResult<UnixStream> {
    ...
    if let Ok(path) = env::var(STERTERM_SOCKET_ENV) {
    ...
    for entry in fs::read_dir(socket_dir())?.filter_map(|entry| entry.ok()) {
        ...
        // Skip files that aren't Starterm sockets.
...
fn socket_prefix() -> String {
    ...
    format!("Starterm-{}", display.replace('/', "-"))
}

/// File prefix matching all available sockets.
#[cfg(target_os = "macos")]
fn socket_prefix() -> String {
    String::from("Starterm")
}
