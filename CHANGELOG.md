...
All notable changes to Starterm are documented in this file.
The sections should follow the order `Packaging`, `Added`, `Changed`, `Fixed` and `Removed`.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

Notable changes to the `starterm_terminal` crate are documented in its
[CHANGELOG](./starterm_terminal/CHANGELOG.md).
...
- Config option `window.level = "AlwaysOnTop"` to force Starterm to always be the toplevel window
- Escape sequence to move cursor forward tabs ( CSI Ps I )
- Pass activation token in `starterm msg create-window` on Wayland/X11
...
- `starterm migrate` crashing with recursive toml imports
...
- `starterm migrate` support for TOML configuration changes
- Headless mode using `starterm --daemon`
...
- Starterm not being properly activated with startup notify
...
- Autokey no longer working with starterm on X11
...
- `starterm migrate` will ignore null values in yaml instead of erroring out

### Fixed

- `starterm migrate` failing with nonexistent imports
...
- `--option` argument for `starterm msg create-window`
...
- Configuration file now uses TOML instead of YAML
    Run `starterm migrate` to automatically convert all configuration files
...
- Blocking paste freezing starterm on Wayland
...
- On macOS, Starterm now reads `AppleFontSmoothing` from user defaults to control font smoothing
...
- Windows `Open Starterm Here` on root of drive displaying error
- On macOS, `font.use_thin_strokes` did not work since Big Sur
- On macOS, trying to load a disabled font would crash
- On macOS, Starterm sessions did not appear in the list of tty sessions for `w` and `who`
...
- New `extra/starterm-msg.man` manpage for the `starterm msg` subcommand
...
- Support for running multiple windows from a single Starterm instance (see docs/features.md)
...
- Newly installed fonts not rendering until Starterm restart
...
- macOS keybinding (cmd+alt+H) hiding all windows other than Starterm
...
- Starterm failing to start on X11 with invalid DPI reported by XRandr
...
- Escapes `CSI Ps b` and `CSI Ps Z` with large parameters locking up Starterm
...
- No live config update when starting Starterm with a broken configuration file
- PTY not drained to the end with the `--hold` flag enabled
- High CPU usage on BSD with live config reload enabled
- Starterm not discarding invalid escape sequences starting with ESC
...
- Updated `setab`/`setaf` capabilities in `starterm-direct` to use colons
...
- Starterm crashing at startup when the configured font does not exist
...
- Capitalized the Starterm.desktop file
...
- Font size resetting when Starterm is moved between screens
- Limited payload length in clipboard escape (used for Tmux copy/paste)
- Starterm not ignoring keyboard events for changing WM focus on X11
...
- Blurred icons in KDE task switcher (starterm.ico is now high-res)
...
- Deadlock on Windows when closing Starterm using the title bar "X" button (ConPTY backend)
...
- Config is not generated anymore, please consider distributing the starterm.yml as documentation
- Removed Starterm terminfo from .deb in favor of ncurses provided one
...
- New CLI flag `--hold` for keeping Starterm opened after its child process exits
- Escape sequence to save and restore window title from stack
- Alternate scroll escape sequence (`CSI ? 1007 h` / `CSI ? 1007 l`)
- Print name of launch command if Starterm failed to execute it
...
- GUI programs launched by Starterm starting in the background on X11
- Text Cursor position when scrolling
- Performance issues while resizing Starterm
...
- On macOS, Starterm will now fallback to Menlo if a font specified in the config cannot be loaded
...
- Starterm turning transparent when opening another window on macOS with chunkwm
...
- Add appstream metadata, located at /extra/linux/io.starterm.Starterm.xml
- The xclip dependency has been removed
- On macOS, Starterm now requests NSSystemAdministrationUsageDescription to
...
- Current Git commit hash to `starterm --version`
...
- On Windows, Starterm will now use the native DirectWrite font API
...
- Starterm now has a fixed minimum supported Rust version of 1.31.0
...
- On Linux, the .desktop file now uses `Starterm` as icon name, which can be
    found at `extra/logo/starterm-term.svg`
...
- Text will reflow instead of truncating when resizing Starterm
...
- Fix panic which could occur when quitting Starterm on Windows if using the Conpty backend
- Automatic copying of selection to clipboard when mouse is released outside of Starterm
...
- Fix lingering Starterm window after child process has exited
...
- Incorrect font spacing after moving Starterm between displays

## Version 0.2.8

### Added

- Window class on Wayland is set to `Starterm` by default
- Log file location is stored in the `STERTERM_LOG` environment variable
...
- Reliably remove log file when Starterm is closed and persistent logging is disabled
...
- Crash when trying to start Starterm on Windows
...
- Prevent Starterm from crashing when started on a system without any free space
- Resolve issue with high CPU usage after moving Starterm between displays
...
- Windows and macOS configuration files (`starterm.yml` is now platform independent)
...
- New mouse and key action `SpawnNewInstance` for launching another instance of Starterm

### Changed

- Log messages are now consistent in style, and some have been removed
- Windows configuration location has been moved from %USERPROFILE%\starterm.yml
    to %APPDATA%\starterm\starterm.yml
...
- Fix panic after quitting Starterm on macOS
...
- Option for maximizing starterm on start (`window.start_maximized`)
- Display notice about errors and warnings inside Starterm
...
- New configuration option `persistent_logging` and CLI flag `--persistent-logging`,
    for keeping the log file after closing Starterm
...
- Starterm now requests privacy permissions on macOS

### Changed

- Extra padding is not evenly spread around the terminal by default anymore
- When the config file is empty, Starterm now logs an info instead of an error message

### Fixed

- Fixed a bad type conversion which could cause underflow on a window resize
- Starterm now spawns a login shell on macOS, as with Terminal.app and iTerm2
...
- DEB file adds Starterm as option to `update-alternatives --config x-terminal-emulator`
...
- On macOS, Starterm will now use the integrated GPU again when available
...
- New terminfo entry, `starterm-direct`, that advertises 24-bit color support
...
- First click on unfocused Starterm windows is no longer ignored on platforms other than macOS
- Reduce memory usage significantly by only initializing part of the scrollback buffer at startup
- The `starterm` terminfo entry no longer requires the `xterm` definition to be
  present on the system
- The default `TERM` value is no longer static; the `starterm` entry is used if
...
- The terminfo entry `starterm-256color`. It is replaced by the `starterm`
...
- When growing the window height, Starterm will now try to load additional lines out of the
  scrollback history
...
- Prevent logger from crashing Starterm when stdout/stderr is not available
