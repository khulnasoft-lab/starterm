...
use starterm_workflows_types::Workflow;
use std::ffi::OsStr;
use std::fs::File;
use std::io::Write;
...
fn main() -> Result<()> {
    ...
    Command::new("cargo").args(["fmt", "-p", "starterm-workflows"]).output()?;
...
/// ```ignore
/// use starterm_workflows_metadata::*;
...
fn write_workflow(workflow: Workflow, file_name: &str) -> Result<()> {
    ...
    writeln!(&module, "use starterm_workflows_types::*;")?;
