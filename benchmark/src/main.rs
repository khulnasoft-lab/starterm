...
const REPO_WHITELIST: [&str; 1] = ["starterm/starterm"];

/// Text identifying that the bot has been mentioned.
const BOT_MENTION: &str = "@starbench";

#[tokio::main]
async fn main() {
    // Setup logging.
    let directives = env::var("RUST_LOG").unwrap_or("warn,starbench=debug,rocket=info".into());
    ...
    thread::spawn(watch_starterm_master);
...
fn watch_starterm_master() -> ! {
    let repository = Repository::get("starterm", "starterm").expect("repo not found");

    let mut connection = model::db_connection();

    loop {
        process_starterm_master(&mut connection, &repository);
        thread::sleep(NOTIFICATION_FREQUENCY);
    }
}

/// Check the starterm master for new updates.
fn process_starterm_master(connection: &mut SqliteConnection, repository: &Repository) {
